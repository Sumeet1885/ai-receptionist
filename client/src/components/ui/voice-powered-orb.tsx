...
interface VoicePoweredOrbProps {
  ...
  voiceLevelOverride?: number; // Added to allow feeding direct Web Audio API inputs if needed
}
...
export const VoicePoweredOrb: FC<VoicePoweredOrbProps> = ({
  ...
  const analyzeAudio = () => {
    if (voiceLevelOverride > 0) return voiceLevelOverride;
  ...
  useEffect(() => {
    ...
    try {
      ...
      const update = (t: number) => {
        ...
        if (voiceLevelOverride > 0) {
          voiceLevel = voiceLevelOverride;
          if (onVoiceDetected) {
